...
class OnboardingStoreRequest extends FormRequest
{
    ...
    {
        $isManagerRegistration = $this->routeIs('api.tenant.onboarding.stores');

        return [
            'plan_slug' => ['required', 'string', Rule::exists('plans', 'slug')->where('is_active', true)],
            'seller_code' => ['nullable', 'string', Rule::exists('users', 'seller_code')->where('role', 'seller')],
            ...
            'owner.password' => $isManagerRegistration
            ...
            'store.business_type' => ['required', 'string', Rule::in(Store::BUSINESS_TYPES)],
    ...
    public function messages(): array
    {
        return [
            ...
            'store.business_type.required' => 'Selecione o tipo de loja.',
            'store.business_type.in' => 'Tipo de loja inválido.',
